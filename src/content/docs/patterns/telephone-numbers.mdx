...
In iOS, Safari automatically detects phone numbers on web pages and converts them into clickable links. This feature is designed to make it easier for users to call a phone number directly from a webpage. However, this automatic behaviour can sometimes interfere with the intended design or functionality of a webpage.

To prevent Safari from automatically detecting and linking phone numbers, a format-detection meta tag can be used in the HTML head section:
...
By adding this meta tag, it disables the automatic phone number detection for the entire page. Then, where it is required to include a clickable phone number link, it can be manually created using `tel:`:
