...
| <span class="marker">C</span> | Anonymous slot | A single, short paragraph works best. Copy length should not exceed two paragraphs of 3 lines. Include emphasis with `<strong>`, inline links `<a>`, lists `<ul>` `<ol>`. Add [caveat link](/components/ns-caveat) if required. |
