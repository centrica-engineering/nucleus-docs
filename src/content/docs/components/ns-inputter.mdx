...
- Refer to the `text` `<ns-inputter>` [autocomplete](#autocomplete) definitions.
- Try to have a default selected `radio` button.
:::

:::dont
- Use spell check if asking for a first and last name within a `text` `<ns-inputter>`.
:::
...
<Figma name={componentName} /> (For text, email, tel, number, separator, mask, search, select, and textarea types).

[Component configuration instructions for radio and checkbox types](https://www.figma.com/design/HdnAatpIIliZhoj6C77aHx/Nucleus-BritishGas---Components?m=auto&node-id=1283-39959&t=0cyP8uC7iqnlpc2w-1)
