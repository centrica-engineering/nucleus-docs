...
The RFC (request for comments) is where the gaps in the design system are identified. We encourage everyone to comment and contribute to a Proposal, with enough community involvement a pattern will hopefully appear, we can then identify and articulate the problem the team is interested in solving.
...
Any one contributing to Nucleus can [create a Proposal](https://github.com/ConnectedHomes/nucleus/issues/new?assignees=&labels=RFC%2C+draft&template=b--proposal.md&title=%5BRFC%5D+Title+of+the+request) in our Github repository.
