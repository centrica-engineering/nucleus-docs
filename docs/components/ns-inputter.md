* # ns-inputter
  ...
  * Atomic type: \(molecule\)
  ...
  | **Type**        | array                                                        |
  | **Default**     | n/a                                                          |
  | **Options**     | isRequired, isNumber,  isInteger, isPostcode, isFirstName, isLastName, isTitle, isEmail, isPhoneNumber, isMobileNumber, isDateOfBirth, isLengthGT(minimum length), isLengthLT(maximum length) |
  ...
  isMinLength(x) - Shows error if the input value is less than x chars.

  isMaxLength(x) - Shows error if the input value is greater than x chars.
